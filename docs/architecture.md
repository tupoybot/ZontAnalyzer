...
Обновлено: 2026-09-24. Архитектура и карта компонентов для разработки.
Ветка М4 переводит приложение на YDB; общая приёмка ещё выполняется.
Действующий production остаётся на SQLite-выпуске `old-stable` до переключения М8.
...
остаются под контролем приложения; AI связывает свидетельства и рассматривает объяснения;
владелец принимает решения, а каналы доставки лишь предоставляют результат.

## Данные и границы

ZONT → адаптер только для чтения → нормализация и YDB → детерминированные метрики и
...
YDB хранит исходную историю, результаты, пользовательские данные и кэши.
Импорт прежней SQLite выполняется отдельным инструментом из online backup.
...
Возможное развитие описано в [backlog публикации](./reports/publication-backlog.md).

Показания газа относятся к установке и выбранной дате снятия, а `report_id`
сохраняет контекст ввода. Наличие отчёта за дату снятия не требуется. Уникальность
по установке и дню сохраняется; перенос редактирует ту же запись с аудитом до/после,
конфликт дат отклоняется. Создание и редактирование различаются явно, чтобы новая
запись не перезаписала существующую. Время снятия неизвестно; интервалы калибровки
используют выбранные даты с прежней неопределённостью границ суток. Газовый PUT
не ждёт публикации: сохранённое состояние читает ближайший успешный цикл worker.
...
| YDB | Прямые транзакции и постраничное чтение: [адаптеры](../src/zont_analyzer/adapters/ydb/); версия структуры — [schema.py](../src/zont_analyzer/adapters/ydb/schema.py) |
| AI | Вызов провайдера и проверка ответа: [provider.py](../src/zont_analyzer/adapters/openai/provider.py); учёт обращений — [ai_usage.py](../src/zont_analyzer/adapters/ydb/ai_usage.py) |
...
HTTPX, Jinja2, SDK YDB и OpenAI. Локальная проверка выполняется в Docker;
облачный кандидат запускается в Serverless Containers.

## Хранение и происхождение данных

Новая облачная версия использует YDB без ORM и преобразования запросов SQLite.
Состояние задания, покрытие истории, отчёты и пользовательские данные переживают
завершение контейнера. Запись результата проверяет владельца задания и версию
исходных данных в той же транзакции. Изменение версии схемы требует явной миграции;
неизвестная версия отклоняется до создания недостающих таблиц.

Выбор облачного хранилища и ограничения нагрузки описаны в
[контракте М0](./cloud/cloud_first_m0.md). Реальную производительность и совпадение
детерминированных фактов проверяют локально на отдельной копии данных перед
облачным smoke. Приёмка полного переноса отслеживается в [М4](./cloud/cloud_first_m4.md).
...
окон для поздних измерений и событий — по умолчанию 120 минут. Повторная загрузка
сохраняет канонические ключи точек и событий. Курсор продвигается только после
успешной записи окна; после ошибки следующий цикл начинает от сохранённого
курсора с перекрытием, даже если простой API длился больше двух часов. У событий
свой курсор: ошибка их загрузки не теряет пропущенный интервал. Явный backfill
повторяет выбранную историю, включая события, независимо от текущего курсора.
...
Последний дневной отчёт и новые, ещё не опубликованные отчёты имеют приоритет
перед обслуживанием старого архива; размер пакета публикации остаётся ограниченным.
...
не следует путать с транзакциями YDB или общей очередью внешних уведомлений.
...
Это относится и к подписям планировщика недельных, месячных и сезонных отчётов:
совпадение прежней подписи позволяет заменить её точной без повторного анализа.
...
Память аналитика — сохранённые структурированные факты и исходы в YDB,
...
фактической модели сохранённого анализа — самостоятельная [доработка 9.2](./ai/model-maintenance-plan.md).
...
[Контракт 9.1](./gas/gas-cost-plan.md) добавляет историю месячной цены/валюты и денежный
...
только [локально в Docker](./development/container-development.md), исходники read-only.
...
[процедуре приёмки](./development/release-process.md) и [инструкции эксплуатации](../deploy/OPERATIONS.md).
