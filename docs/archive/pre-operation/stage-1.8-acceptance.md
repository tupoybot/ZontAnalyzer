...
## Уточнение интерфейса и AI-отчёт после приёмки

По обратной связи владельца календарь уплотнён без изменения навигации: меньше ячейки,
кнопки и интервалы, на широком экране переключатели объединены в одну строку. На реальном
отчёте высота календаря — 233 px при ширине окна 1036 px и 264 px при 390 px;
горизонтальной прокрутки нет, включая длинные ID/evidence в AI-карточках.

Отсутствующий MTTR теперь явно отображается в HTML/text как «нет достоверных данных»
с причиной. В текущем отчёте у обоих подтверждённых отказов момент восстановления попал
в разрыв наблюдаемости. Число не выдумывается; при наблюдаемом восстановлении сохраняется
обычный числовой MTTR. Семантика reliability и canonical JSON не изменена.

С отдельного разрешения владельца локально на свежей online-копии production БД
перегенерирован дневной отчёт за 2026-09-04: один AI-запрос без retries, `ai_used=true`,
качество 87,42%, две рекомендации, 11592 input / 1436 output tokens. Сохранены готовый
