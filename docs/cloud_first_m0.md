...
При запуске приложение будет выбирать по настройкам, какую базу данных использовать
(SQLite или YDB), куда публиковать отчёты, как принимать задания, откуда получать
настройки и секреты и какие запросы направлять через прокси. За это будет отвечать
отдельный код запуска. Расчёты и анализ данных останутся общими: они не должны
зависеть от библиотек конкретной БД или способа авторизации в облаке.

Сейчас [код запуска приложения](../src/zont_analyzer/runtime.py) сразу подключает
базу данных, обновляет её структуру и статусы рекомендаций. Поэтому его нельзя
использовать только для проверки настроек и прав доступа: вместе с проверкой
начнётся работа с данными. В M4 эти действия нужно разделить, чтобы настройки
и доступ можно было проверить отдельно, без изменения БД и рекомендаций.
