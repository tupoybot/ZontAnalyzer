...
M0 подготовлен к приёмке; уточнение сбора телеметрии от 2026-09-15:
...
## M2 — Проверить задание отчёта в Serverless Container и границы scratch/state
...
- Запускать выбранное report/publication/recovery задание через invocation adapter;
  повторный вызов прогретого экземпляра тоже обрабатывается. Report job сам
  получает нужную телеметрию, отдельный периодический sync не требуется.
- Зафиксировать сбор телеметрии перед daily report: weekly/monthly/quarterly
  используют сохранённые samples и догружают только недостающие диапазоны.
  Проверить 90-дневную границу ZONT, долговечность собственной истории,
  successful empty range против gap/error и пометку unrecoverable для старого
  диапазона, которого нет в локальном хранилище.
- Проверить catch-up от самого старого recoverable диапазона и bounded late-data
  refresh по overlap; полный refetch уже сохранённого периода запрещён.
- Проверить первый запуск, простой и квартал 91–92 дня: доступная история
  загружается до её исчезновения у источника, старые данные читаются из нашей БД,
  незагруженные окна у границы retention вызывают предупреждение. HTTP 200,
  обрезанный ответ и отдельные samples не заменяют подтверждённое покрытие окна.
...
- Acceptance включает доказательство, что один и тот же weekly/monthly (и
  planned quarterly) диапазон не загружается повторно при наличии samples,
  а неполное покрытие и unrecoverable диапазоны видны в результате отчёта.
...
2. time-range query samples для одного/нескольких series, включая durable
   coverage markers, successful empty ranges, gaps и 90-дневную source boundary;
...
- Contract tests доказывают idempotent missing-range ingestion, отсутствие
  дублирующего fetch для weekly/monthly/planned quarterly, bounded late-data
  refresh и сохранение coverage дольше 90 дней источника.
...
- Перенести в оба adapter долговечную семантику telemetry coverage: samples
  сохраняются без TTL ZONT, missing ranges догружаются oldest-first, empty-success
  не превращается в gap, а unrecoverable диапазон сохраняется как явный результат.
...
- При повторной приёмке одинаковое окно отчёта не вызывает повторный полный fetch;
  quarter-long period за пределами 90 дней строится только из ранее сохранённых
  samples, а недоступные старые интервалы остаются явными coverage gaps.
...
- Подтвердить готовность M0–M6, доступ к DNS/TLS и явно учтённые зависимости
  выбранного egress (включая существующий внешний proxy);
  проверить отказ/восстановление proxy и сохранение его политики.
...
и молчание её не заменяют. Перед merge — локальный `docker system prune -af`, не на production host.
