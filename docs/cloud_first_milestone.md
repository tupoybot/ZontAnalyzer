...
Текущий план предусматривает общий прикладной контракт хранилища и две его
реализации: SQLite и YDB (если принята в M3). Каждый экземпляр приложения выбирает
один backend при запуске; двойной записи и постоянной синхронизации двух БД нет.
Поддержка двух реализаций всё же требует отдельных миграций и интеграционных
проверок, поэтому её срок должен быть осознанным решением.
...
границы транзакций, контракт выбора backend и решение о сроке поддержки SQLite.
...
В M0 явно выбрать: сохранять SQLite как поддерживаемый offline backend после
миграции либо оставить его на период перехода/rollback и затем вывести из поддержки.
До принятия такого решения действующий SQLite runtime сохраняется. Локальная YDB
позволяет выбрать второй вариант; отсутствие локальной YDB не является причиной
бессрочной поддержки SQLite. До окончания rollback-периода действуют проверки
совместимости обоих adapters и контракт единого application image.
...
- Определены операции/типы storage contracts, выбор backend и срок поддержки
  SQLite; различия локальной и managed YDB перечислены как отдельные проверки.
...
Начать с YDB в локальном Docker по порядку из M0; затем проверить выбранный
managed backend в folder `zontanalyzer`. Результаты локальных функциональных
