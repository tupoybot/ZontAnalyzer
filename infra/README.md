...
Создание ветки не требует обновления этих секретов. В ручном запуске новый
проверенный образ можно выбрать параметром `application_image` по digest;
`M1_CONFIG_JSON` обновляется при изменении самой конфигурации окружения.
