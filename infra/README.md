...
Обычный push проверяет план. Ручной запуск с `apply=true` применяет его и выполняет smoke.
Ручной dispatch требует наличия workflow в default branch GitHub; его публикация
туда проходит отдельную приёмку, без обхода запрета на merge. До этого для проверки
apply из инфраструктурной ветки используется явный префикс сообщения коммита
`[m1-apply] `. Такой push выполняет тот же полный путь plan/apply и smoke с
