...
записывает в локальную БД, что владелец сам сделал изменение. `reject` сохраняет пояснение
владельца; компактные результаты `applied`/`rejected` передаются в следующие AI-анализы,
чтобы не повторять отклонённую гипотезу без новых противоречащих данных.
