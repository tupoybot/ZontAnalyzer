...
2. MTBF и среднее восстановление связи котла (MTTR/MTBR), если есть завершённые интервалы;
...
контроллера. Потеря сети 220 В не считается отказом котла и не сбрасывает uptime ZONT, пока
встроенная батарея поддерживает устойчивую телеметрию. Из ZONT запрашиваются только нужные
типы исходных событий; координаты и посторонние события не сохраняются и не отправляются AI.
