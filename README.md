...
Этапы до 8.5 и доработка 9.1 (стоимость газа) приняты владельцем и слиты в `main`. Текущий рабочий
цикл — этап 9: опытно-промышленная эксплуатация и доработки. Этап 9.2 (сопровождение
моделей AI) реализуется и проверяется в `stage9/ai-models`. Этапы 10 (многокомнатность) и 11
...
вероятный штатный цикл антилегионеллы. При активном ГВС таким же циклом считается
подтверждённый пламенем быстрый разгон бака до 55–65 °C при неизменной за 15 минут
до старта и во время нагрева уставке ниже 55 °C и превышении её минимум на 3 °C.
Изменение уставки и штатная уставка 55 °C такой вывод исключают. Короткий флаг
пламени без последующего роста температуры подачи фиксируется как шум и не входит
в статистику циклов горелки.
...
В разделе «AI» настроек отчёта можно изменить модели дневного анализа и обзоров,
их глубину рассуждения и включение AI. Веб-настройки имеют приоритет над YAML;
их можно сбросить, изменения сохраняются в аудите. Новый выбор действует только
для новых анализов. В шапке отчёта указана модель сохранённого AI-ответа; старые
ответы без достоверного происхождения помечены «Модель не сохранена».

По умолчанию раз в 60 дней отдельная задача проверяет официальный каталог моделей
и сроки прекращения поддержки. Интервал можно изменить, проверку отключить или
запустить вручную. Предложение принимается, отклоняется или откладывается владельцем;
автоматической смены модели нет. Подробности — в [контракте 9.2](docs/model-maintenance-plan.md),
пакеты и команды локального сравнения — в [оценке моделей](docs/model-evaluation.md).
