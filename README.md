...
Сейчас это рабочий CLI-MVP, проверенный на реальной установке BAXI Connect+. Web-интерфейса и
Telegram пока нет. Отопление, режимы и автоматический летний переход уже анализируются;
по ГВС пока считаются только циклы горелки. Температура БКН, режим ГВС и его влияние на
отопление — ближайший аналитический этап в [актуальном плане](.codex/roadmap.md).
...
4. циклы горелки отдельно для отопления и ГВС;
5. события смены режима/уставки и обнаруженные отклонения;
6. безопасные рекомендации с evidence ID, рисками и условиями остановки.
...
Остановка — `Ctrl+C`. Worker синхронизирует данные с интервалом из конфигурации и один раз
создаёт daily-отчёт за вчера. Сейчас он не экспортирует `latest.html` автоматически и не
восстанавливает все пропущенные суточные отчёты; это часть ближайшего product-пилота. Для
постоянного запуска используйте Docker или менеджер сервисов только после успешного
`run --once`.
...
reports/
  latest.html                   экспорт создаётся указанной командой
```
...
Экспортировать HTML из контейнера на хост:

```bash
docker exec zont-analyzer zont-analyzer --data-dir /data \
  report export --format html -o /data/latest.html
docker cp zont-analyzer:/data/latest.html reports/latest.html
```
...
| `run [--once]` | Синхронизировать данные и запускать daily-анализ. |
