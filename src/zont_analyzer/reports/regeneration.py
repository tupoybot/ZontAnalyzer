...
<script>(function() {{
  ...
  button.addEventListener('click', async () => {{ requested = true; button.disabled = true; show('queued'); try {{ const r = await fetch(url, {{method:'POST', credentials:'same-origin', body:'{{}}', headers:{{'Content-Type':'application/json'}}}}); const v = await r.json(); if (!r.ok) throw new Error(v.error || 'HTTP ' + r.status); show(v.status, v.error); }} catch (error) {{ show('error', error.message); button.disabled = false; return; }} poll(); }});
  poll();
}})();</script>'''
